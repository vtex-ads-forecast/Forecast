...
def _months_marcados_current(html):
    """Chaves de mes com status current DENTRO de MONTHS_DATA."""
    W = _ws()
    md = re.search("const MONTHS_DATA" + W + "=" + W + "[{]", html)
    escopo = html[md.start():] if md else html
    return re.findall('"([0-9]{4}-[0-9]{2})":' + W + "[{][^}]*status:" + W + '"current"', escopo)


def get_current_month_from_html(html):
    """Mes corrente = o MAIS RECENTE marcado como current.

    Antes devolvia a PRIMEIRA ocorrencia do arquivo inteiro. Quando o step 1 da
    virada falhava em silencio, o mes antigo seguia marcado como current e vinha
    primeiro -- entao a virada disparava DE NOVO a cada run, e cada disparo
    criava outro const REAL_CLOSED_<mes> (SyntaxError = dashboard fora do ar),
    zerava o ACTUALS e resetava o REAL_APRIL. Incidente de 01-04/09/2026.
    """
    achados = _months_marcados_current(html)
    return max(achados) if achados else None


def normaliza_current(html):
    """Auto-cura: so o mes mais recente pode estar como current."""
    achados = _months_marcados_current(html)
    if len(achados) <= 1:
        return html
    W = _ws()
    mais_novo = max(achados)
    for mk in achados:
        if mk == mais_novo:
            continue
        pat = re.compile('("' + re.escape(mk) + '":' + W + "[{][^}]*status:" + W + ')"current"')
        html, n = pat.subn(lambda m: m.group(1) + '"closed"', html, count=1)
        if n:
            print("  OK " + mk + " estava como current -> corrigido para closed")
    return html
...
def close_month_and_open_new(html, new_month_key):
    ...
    Steps:
      1. Mark current month as 'closed' in MONTHS_DATA
      2. Copy REAL_APRIL → REAL_CLOSED_NEW and add to CLOSED_DETAIL
      3. Build cumulative daily arrays for closed month
      4. Reset REAL_APRIL to zeros
      5. Reset NA, TD, ACTUALS
      6. Reset REAL_DAILY_APR
      7. Reset ADV_DATA
      8. Add new month entry to MONTHS_DATA
    """
    cur_month = get_current_month_from_html(html)
    if not cur_month:
        print("  ⚠ Could not find current month in MONTHS_DATA. Skipping transition.")
        return html

    new_year = int(new_month_key[:4])
    new_mon = int(new_month_key[5:7])
    new_td = days_in_month(new_year, new_mon)
    new_label = f"{MONTH_NAMES[new_mon]} {new_year}"

    cur_mon = int(cur_month[5:7])
    ...
    # Compute blended TR from current REAL_APRIL for cumulative rev
    ra_tr = re.search(r'const REAL_APRIL\s*=\s*\{', html)
    blended_tr = 0.1  # fallback
    if ra_tr:
        brace = html.find('{', ra_tr.start())
        d = 0
        block_end = brace
        for i in range(brace, len(html)):
            if html[i] == '{': d += 1
                ...
                if d == 0:
                    block_end = i + 1
                    break
        ra_block_text = html[ra_tr.start():block_end]
        sp_total = sum(int(x) for x in re.findall(r'spendReal:(\d+)', ra_block_text))
        rv_total = sum(int(x) for x in re.findall(r'revReal:(\d+)', ra_block_text))
        if sp_total > 0:
            blended_tr = rv_total / sp_total

    # --- Step 1: Mark current month as 'closed' ---
    actuals_m = re.search(r'const ACTUALS\s*=\s*\[(.*?)\];', html, re.DOTALL)
    actuals_total = 0
    if actuals_m and actuals_m.group(1).strip():
        for am in re.finditer(r'"adspend":\s*(\d+)', actuals_m.group(1)):
            actuals_total += int(am.group(1))

    # Read current month's TD and META values before resetting
    cur_td_m = re.search(r'const TD\s*=\s*(\d+)', html)
    cur_td = int(cur_td_m.group(1)) if cur_td_m else 31
    cur_na_m = re.search(r'const NA\s*=\s*(\d+)', html)
    cur_na = int(cur_na_m.group(1)) if cur_na_m else cur_td
    ...
    # Find the current month entry within MONTHS_DATA and replace it entirely
    # Look for the entry: "2026-05": { status: "current" ... }
    md_m = re.search(r'const MONTHS_DATA\s*=\s*\{', html)
    md_start = md_m.start()
    # Find this month's entry within MONTHS_DATA
    entry_pattern = rf'"{re.escape(cur_month)}":\s*\{{'
    entry_m = re.search(entry_pattern, html[md_start:])
    if entry_m:
        abs_entry_start = md_start + entry_m.start()
        # Find the closing brace of this entry
        brace_start = html.find('{', abs_entry_start + len(cur_month))
        d = 0
        entry_end = brace_start
        for i in range(brace_start, len(html)):
            if html[i] == '{': d += 1
                ...
                if d == 0:
                    entry_end = i + 1
                    break
        # Build the closed entry with literal values
        closed_entry = (
            f'"{cur_month}": {{\n'
            f'    status: "closed", label: "{MONTH_NAMES[cur_mon]} {cur_month[:4]}", '
            f'days: {cur_td}, actualDays: {cur_na},\n'
            f'    metaSpend: {meta_sp}, metaRev: {meta_rv},\n'
            f'    realSpend: {actuals_total},\n'
            f'    realRev: {actuals_rev},\n'
            f'    segMetas: null,\n'
            f'    others: null,\n'
            f'    forecastUsed: null\n'
            f'  }}'
        )
        html = html[:abs_entry_start] + closed_entry + html[entry_end:]
        print(f"  ✓ Closed {cur_month} in MONTHS_DATA (realSpend={actuals_total:,})")
    else:
        print("  ERRO: nao achei a entrada de " + cur_month + " em MONTHS_DATA"
              " -- o mes NAO foi fechado. Abortando a virada para nao duplicar.")
        raise RuntimeError("virada abortada: " + cur_month + " nao encontrado")
    ...
    ra_value = ra_block[eq_pos + 1:].strip()
    ...
    new_month_entry = (
        ...
        f'    realSpend: 0, realRev: 0,\n'
        f'    real: REAL_APRIL, daily: REAL_DAILY_APR\n'
        f'  }}'
    )
    html = html[:md_end] + new_month_entry + html[md_end:]
...
def main():
    ...
    # 3.5. Check for month transition
    html = normaliza_current(html)
    data_month_key = f"{start_date[:4]}-{start_date[5:7]}"
    current_month_key = get_current_month_from_html(html)
    _W = _ws()
    _md = re.search("const MONTHS_DATA" + _W + "=" + _W + "[{]", html)
    _escopo = html[_md.start():] if _md else html
    _ja = re.search('"' + re.escape(data_month_key) + '":' + _W + "[{]", _escopo) is not None
    if current_month_key and data_month_key != current_month_key and _ja:
        print("  AVISO: " + data_month_key + " ja existe em MONTHS_DATA -- "
              "virada ja feita, pulando")
    elif current_month_key and data_month_key != current_month_key:
        print(f"\n⚡ Month change detected: dashboard={current_month_key}, data={data_month_key}")
        html = close_month_and_open_new(html, data_month_key)
        # Save intermediate HTML so transition persists even if apply_updates fails
        with open(HTML_PATH, "w", encoding="utf-8") as f:
            f.write(html)
        print("  ✓ Transition saved to HTML")
        # Re-extract pub mappings from the reset HTML
        pub_seg, pub_tr = extract_pub_mapping(html)
...
def get_current_month_from_html(html):
    """Extract the current month key (e.g. '2026-05') from MONTHS_DATA."""
    m = re.search(r'"(\d{4}-\d{2})":\s*\{[^}]*status:\s*"current"', html)
